...
export class CartStateService {
  ...
  private add(state: Signal<State>, product: ProductItemCart): Partial<State> {
    ...
    const isInCart = this.state().products.find(
      (productInCart: { product: { id: number; }; }) => productInCart.product.id === product.product.id,
    );
  ...
  private update(state: Signal<State>, product: ProductItemCart) {
    ...
    const products = state().products.map((producInCart) => {
      if(producInCart.product.id === product.product.id) {
        return {...producInCart, quantity: product.quantity}
      }
